...
- 收到任务后单并发执行，实时回传步骤状态。
- Cookie、Token、EncodingAESKey、kitsecret 等敏感明文不上传，只允许脱敏摘要。
...
Windows 调试时发送脱敏诊断摘要。该消息只能包含环境、进程、网络、登录态、artifact 索引和最近错误摘要，不允许包含 Cookie、Token、EncodingAESKey、kitsecret、页面原始截图或完整日志。
...
- 支持本地重放最后一条 `task.dispatch`，但默认 dry-run，真实写入必须显式确认。
- 支持记录屏幕分辨率、DPI 缩放、当前 Windows session、进程身份和浏览器 Profile 路径。
- 诊断摘要不得包含 Cookie、Token、EncodingAESKey、kitsecret、完整 HTML、完整截图或请求头明文。
