...
- **群主筛选** - 下拉框筛选特定群主的任务
- **重试按钮** - 仅在满足条件时可用（状态失败 + 操作说明为"点击'+'创建群" + 异常类型为 ImageNotFoundException）
- **自动刷新** - 默认每 5 秒自动刷新数据

### 5. 队列监控 API

**获取队列统计**: `GET /api/queue/stats`

**获取任务历史**: `GET /api/queue/history?limit=50&offset=0`

**获取任务详情**: `GET /api/queue/task/{task_id}`

**重试任务**: `POST /api/queue/task/{task_id}/retry`
...
```json
{
  "msgtype": "text",
  "text": {
    "content": "xx有限公司在「点击更多按钮发送二维码」过程中遇到问题，请对应技术支持及时处理",
    "mentioned_mobile_list": ["18812345678", "18852645418"]
  }
}
```
